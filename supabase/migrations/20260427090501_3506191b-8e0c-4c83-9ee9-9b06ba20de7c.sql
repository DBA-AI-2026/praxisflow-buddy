-- Phase 4A Korrektur: convert_reservation_to_lead
-- 1) Mobilnummer ist Pflicht (kein Fake-Fallback mehr)
-- 2) assignment_source unverändert aus Reservierung übernehmen (kein "reservation:"-Präfix);
--    Herkunft wird ausschließlich über source='reservation_conversion' transportiert.
...
BEGIN
  ...
  IF v_mobile IS NULL THEN
    RAISE EXCEPTION 'Mobilnummer ist Pflichtfeld für die Konvertierung' USING ERRCODE = '23502';
  END IF;
  ...
  -- assignment_source: schemakonform aus Reservierung übernehmen.
  -- Erlaubte Werte im bestehenden System: 'plz_auto', 'manual', 'none', 'unassigned'.
  v_assignment := CASE
    WHEN v_res.assigned_ad_id IS NULL THEN 'none'
    WHEN v_res.assignment_source IN ('plz_auto', 'manual', 'none', 'unassigned')
      THEN v_res.assignment_source
    ELSE 'plz_auto'
  END;
