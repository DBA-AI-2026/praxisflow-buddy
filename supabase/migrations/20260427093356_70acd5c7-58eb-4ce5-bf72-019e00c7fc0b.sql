-- 1) Spalte für Produktinteresse an Reservierungen
ALTER TABLE public.praxis_reservations
  ADD COLUMN IF NOT EXISTS interested_products text[] NOT NULL DEFAULT '{}';

-- 2) Erweiterte Konvertierungs-RPC: optionaler Parameter p_interested_products,
--    der Reservierungs-Produkte überschreibt; fallback ist die Liste der Reservierung.
...
BEGIN
  ...
  -- Produktinteresse: Override hat Vorrang, sonst aus Reservierung
  v_products := COALESCE(p_interested_products, v_res.interested_products, '{}'::text[]);
