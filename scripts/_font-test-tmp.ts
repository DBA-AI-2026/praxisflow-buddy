...
const reg  = await doc.embedFont(toU8("/tmp/Exo2-400.ttf"), { subset: true });
const med  = await doc.embedFont(toU8("/tmp/Exo2-500.ttf"), { subset: true });
const bold = await doc.embedFont(toU8("/tmp/Exo2-700.ttf"), { subset: true });
const page = doc.addPage([595, 842]);
let y = 780;
const lines: [string, any, number][] = [
  ["VERTRAGSÜBERSICHT — Exo 2 PoC", bold, 22],
  ["Regular: Müller & Söhne GmbH – Honorarfuchs", reg, 12],
  ["Medium:  1.234,56 € / Monat – ÄÖÜß€", med, 12],
...
console.log("subset PDF bytes:", bytes.length);

const doc2 = await PDFDocument.create();
doc2.registerFontkit(fontkit as any);
await doc2.embedFont(toU8("/tmp/Exo2-400.ttf"));
await doc2.embedFont(toU8("/tmp/Exo2-500.ttf"));
await doc2.embedFont(toU8("/tmp/Exo2-700.ttf"));
doc2.addPage([595, 842]).drawText("x", { x: 50, y: 700, size: 12 });
const b2 = await doc2.save();
console.log("NO-subset (3 cuts fully embedded) PDF bytes:", b2.length);
